...
"""
DICOM Protocol Fuzzer (v2.0 - Refactored Architecture)

A fuzzing tool for testing DICOM SCP implementations using the refactored
dicom module with:
- Native Scapy random generators (RandChoice, RandString)
- Single smart packet classes (no more *_Raw duplicates)
- Explicit command_group_length field for buffer attacks
- Global and per-packet raw_mode for protocol violations

Fuzzing capabilities:
- Buffer over-read attacks via explicit command_group_length
- Odd-length UIDs via raw_mode (global or per-packet)
...
try:
    from dicom import (
        ...
        DICOMMaximumLength,
        # DIMSE Packet classes (Single Smart Classes!)
        C_ECHO_RQ,
        C_STORE_RQ,
        # Helpers
...
def fuzz_association_handshake(session_args):
    ...
    try:
        ...
        if session.connect():
            ...
            aarq = A_ASSOCIATE_RQ(
                called_ae_title=b"X" * 20,  # Overlong!
                calling_ae_title=_pad_ae_title(session_args["calling_ae"]),
                variable_items=variable_items,
            )
            pkt = DICOM() / aarq
            
            session._scu.send(pkt)
            response = session._scu.stream.recv() if session._scu.stream else None
    ...
    try:
        ...
        if session.connect():
            ...
            session._scu.sock.sendall(bytes(raw_bytes))
    ...
    try:
        ...
        if session.connect():
            ...
            session._scu.sock.sendall(unknown_pdu)
    ...
    # --- Test 7: Odd-Length UID (using raw_mode) ---
    script_log.info("Test 7: Odd-Length UID in Application Context (raw_mode)")
    try:
        # Enable global raw_mode for this test
        original_raw_mode = conf.contribs.get("dicom", {}).get("raw_mode", False)
        ...
        if session.connect():
            # Odd-length UID (15 bytes)
            odd_uid = b"1.2.840.10008.1"  # 15 bytes - odd!
            
            variable_items = [
                DICOMVariableItem() / DICOMApplicationContext(uid=odd_uid),
                build_presentation_context_rq(1, VERIFICATION_SOP_CLASS_UID, [DEFAULT_TRANSFER_SYNTAX_UID]),
                build_user_information(max_pdu_length=16384),
            ]
            aarq = A_ASSOCIATE_RQ(
            ...
            results["passed"] += 1
        session.close()
        conf.contribs["dicom"]["raw_mode"] = original_raw_mode
    except Exception as e:
        script_log.info(f"[PASS] Server rejected odd-length UID: {e}")
        results["passed"] += 1
        conf.contribs["dicom"]["raw_mode"] = False
...
def fuzz_cstore_with_file(session_args, dcm_file_path):
    ...
    # --- Test 2: FUZZ command_group_length (Buffer Over-Read Attack!) ---
    script_log.info("Test 2: Fuzz command_group_length (Buffer Over-Read)")
    for fuzz_len in [0, 10, 0xFFFF, 0xFFFFFFFF]:
        script_log.info(f"  Testing command_group_length=0x{fuzz_len:X}")
        try:
            session = DICOMSession(
                dst_ip=session_args["ip"],
                dst_port=session_args["port"],
            ...
            if session.associate(requested_contexts=requested_contexts):
                ...
                if ctx_id:
                    # NEW: Explicit command_group_length field for fuzzing!
                    dimse_cmd = C_STORE_RQ(
                        command_group_length=fuzz_len,  # FUZZED!
                    ...
                    session._scu.send(DICOM() / P_DATA_TF(pdv_items=[cmd_pdv]))
                    ...
                    session._scu.send(DICOM() / P_DATA_TF(pdv_items=[data_pdv]))
    ...
    # --- Test 5: Odd-length UIDs using raw_mode ---
    script_log.info("Test 5: Odd-length UIDs using global raw_mode")
    try:
        original_raw_mode = conf.contribs.get("dicom", {}).get("raw_mode", False)
        conf.contribs["dicom"]["raw_mode"] = True
        ...
        if session.associate(requested_contexts=requested_contexts):
            ...
            if ctx_id:
                # Odd-length UIDs (raw_mode prevents auto-padding)
                dimse_cmd = C_STORE_RQ(
                    affected_sop_class_uid="1.2.3.4.5",  # 9 bytes - odd!
                    affected_sop_instance_uid="1.2.3.4.5.6.7",  # 13 bytes - odd!
                    message_id=1,
                )
    ...
    try:
        ...
        if session.associate(requested_contexts=requested_contexts):
            ...
            if ctx_id:
                ...
                session._scu.send(DICOM() / P_DATA_TF(pdv_items=[partial_pdv]))
...
def main():
    parser = argparse.ArgumentParser(
        description="DICOM Protocol Fuzzer v2.0 (Refactored Architecture)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
New in v2.0:
- Explicit command_group_length field for buffer attacks
- Global/per-packet raw_mode for odd-length UIDs
- Native Scapy fuzz() integration
- Automaton-based session management
...
""",
    ...
    script_log.info(f"=== DICOM Fuzzer v2.0 Started (Mode: {args.mode}) ===")
    ...
    script_log.info("=== DICOM Fuzzer v2.0 Finished ===")
